...
def trajectory_track(r_dot,r_t,theta_g,theta_t,dist_er,theta_ch,r_int_er,r_old_er,theta_int_er,theta_old_er,dev_int_er,dev_old_er):
    Kp_ln, Kd_ln, Ki_ln = 3,1,0.4
    r_er = dist_er
    r_diff_er = r_er - r_old_er
    r_int_er = r_er + r_int_er
    PID_ln = r_dot - 0.3*abs((Kp_ln*r_er + Kd_ln*r_diff_er +Ki_ln*r_int_er/15)/(Kp_ln*4+Kd_ln*2))
    r_old_er = r_er

    Kp_dev, Kd_dev, Ki_dev = 1,0.5,0.1
    dev_er = theta_g - theta_ch
    if abs(dev_er) < 0.5:
        dev_int_er = 0
    ...
    theta_diff_er = theta_er - theta_old_er
    theta_int_er = theta_er + theta_int_er
    PID_wt = ((Kp_ang*theta_er)/math.pi + (Kd_ang*theta_diff_er)/(0.5*math.pi) + (Ki_ang*theta_int_er)/(6*math.pi)) + 0.5*PID_dev
    print theta_er,theta_t,theta_ch
    theta_old_er = theta_er
...
if __name__ == '__main__':
    ...
    r_old_er,theta_int_er,theta_old_er,dev_int_er,dev_old_er,count,r_int_er = 0,0,0,0,0,0,0
    x_act,y_act = [],[]
    vx_act,vy_act = [],[]
    r_now,theta_now = [],[]
    ...
    while not rospy.is_shutdown():
        ...
        while(1):
            if count < int(total_time/sampling_rate):
                ...
                rospy.Subscriber("/odom",Odometry,pose)
                x_r,y_r,theta_r,theta_ch = pose.get_msg()
                dist_er = math.sqrt((x_t[count]-x_r)*(x_t[count]-x_r)+(y_t[count]-y_r)*(y_t[count]-y_r))
                ...
                vx,r_int_er,r_old_er,wt,theta_int_er,theta_old_er,dev_int_er,dev_old_er = trajectory_track(r_dot[count],r_t[count],theta_g,theta_t[count],dist_er,theta_ch,r_int_er,r_old_er,theta_int_er,theta_old_er,dev_int_er,dev_old_er)
