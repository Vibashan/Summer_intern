 ...
 void vel_cb(const geometry_msgs::Twist& cmd)
{
    ...
    vl = (cos((120 + theta)*deg2rad)*vy - sin((120 + theta)*deg2rad)*vx  + L*wt)/(speed);
    vrear = (cos(theta*deg2rad)*vy - sin(theta*deg2rad)*vx + L*wt)/(speed);
    vr = (cos((-120 + theta)*deg2rad)*vy - sin((-120 + theta)*deg2rad)*vx  + L*wt)/(speed);
