 ...
 void vel_cb(const geometry_msgs::Twist& cmd)
{
    ...
    vl = (cos((150 + theta)*deg2rad)*vy - sin((150 + theta)*deg2rad)*vx  + L*wt)/(speed);
    vrear = (cos((30 + theta)*deg2rad)*vy - sin((30 + theta)*deg2rad)*vx + L*wt)/(speed);
    vr = (cos((270 + theta)*deg2rad)*vy - sin((270 + theta)*deg2rad)*vx  + L*wt)/(speed);
